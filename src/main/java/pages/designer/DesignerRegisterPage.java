...
public class DesignerRegisterPage extends Page{
	...
	public DesignerRegisterPage(WebDriver browser, Data data) {
		super(browser, data);
	}
	...
	public void enterFirstDetailsForRegister(HashMap<String, String> details) {
		...
		enterText(txtDesignLabName, "Design Lab Name", details.get("Design Lab Name"));
	...
	public void registerNewUser(Data data) {
		...
		waitForElement(txtFirstNameJoinus);
	...
	public void registerExistingUser() {
		...
		firstDetails.put("DateOfBirth", "06/02/1994");
		firstDetails.put("Email Address", Values.frameworkProperty.getProperty("designer.username"));		
		firstDetails.put("Country", "India");
		firstDetails.put("Mobile Number", "9876541230");
		firstDetails.put("Password", "Password$123");
		firstDetails.put("Confirm Password", "Password$123");
		firstDetails.put("Design Lab Name", "Lab Name");
		firstDetails.put("Business Address", "Automation Tetsing");
		firstDetails.put("Business PostCode", "9875623");
		waitForElement(txtFirstNameJoinus);
		enterFirstDetailsForRegister(firstDetails);		
		clickOn(btnNextAndSubmitJoinus, "Next Button");
		HashMap<String, String> finalDetails = new HashMap<String, String>();
		finalDetails.put("Bank Name", "Auto Bank");
		finalDetails.put("Bank Country", "India");
		finalDetails.put("Account Holder Name", "Auto Designer");
		finalDetails.put("Account Number", "652345789823");
		finalDetails.put("Bank Sort Code", "ABCD");
		finalDetails.put("IBAN", "435TFB");
		enterFinalDetailsForRegister(finalDetails);
		uploadFile(btnProfilePicJoinUs, ".\\files\\dp.jpg", btnConfirmOk);
		jsClick(btnNextAndSubmitJoinus, "Submit Button");
		...
		if(isElementPresent(weSuccErrorPopUp)) {
			...
			// TODO : Also add the validation for error header
			if(text.contains("User already exists")) {
				passed("User already exists message is displayed as expected");
				takeScreenshot();
			}else {
				fail("Error message is not displayed as expected, Expected is User already exists,But actual is "+text);
				takeScreenshot();
			}
		}else {
