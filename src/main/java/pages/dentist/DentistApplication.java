...
public class DentistApplication {
	...
	public DentistApplication(Data data) {
	...
	public DentistLoginPage open32CoApplication() {
		try {
			browser.get(url);
		...
		return new DentistLoginPage(browser, data);
	}
	
	public DentistRegisterPage openDentistJoinus() {
		try {
			String url = Values.frameworkProperty.getProperty("dentist.url.joinus","");
			browser.get(url);
			browser.manage().window().maximize();
