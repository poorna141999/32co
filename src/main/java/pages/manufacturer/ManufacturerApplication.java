...
public class ManufacturerApplication {
	...
	public ManufacturerApplication(Data data) {
	...
	public ManufacturerLoginPage open32CoApplication() {
		try {
			browser.get(url);
		...
		return new ManufacturerLoginPage(browser, data);
	}
	
	public ManufacturerRegisterPage openManufacturerJoinus() {
		try {
			String url = Values.frameworkProperty.getProperty("manufacturer.url.joinus","");
			browser.get(url);
			browser.manage().window().maximize();
		...
		return new ManufacturerRegisterPage(browser, data);
	}
